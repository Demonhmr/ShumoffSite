			...
			<div class="appointment-section__form">
				<form class="appointment-form" method="post" action="<?php echo esc_url( home_url( '/' ) ); ?>" data-validate data-ajax-action="shumoff_appointment" novalidate>
					...
					<!-- Honeypot: скрыто стилями, люди не заполняют.
					     Имя/подпись нарочно не похожи на реальные поля профиля:
					     Chrome игнорирует autocomplete="off" и заполнял бы «Компанию»
					     из сохранённого адреса — терялись бы живые заявки. -->
					<div class="form-group form-group--hp" aria-hidden="true">
						<label for="appt-extra"><?php _e( 'Оставьте это поле пустым', 'shumoff' ); ?></label>
						<input type="text" id="appt-extra" name="appt_extra" tabindex="-1" autocomplete="one-time-code">
					</div>
